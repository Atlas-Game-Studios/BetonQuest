...
@CustomLog
public class SimpleNPCsHologram extends BukkitRunnable {

    private static SimpleNPCsHologram instance;
    ...
    public static void reload() {
        synchronized (SimpleNPCsHologram.class) {
            if (instance != null) {
                instance.cancel();
                instance = null;
    ...
    private Vector getVector(final QuestPackage pack, final String key, final String vector) {
        if (vector != null) {
            try {
                ...
                LOG.warn(pack.getQuestPath() + ": Invalid vector in Hologram '" + key + "': " + vector);
            }
        }
        ...
        if (rawConditions != null) {
            for (final String part : rawConditions.split(",")) {
                try {
                    ...
                    LOG.warn("Error while loading " + part + " condition for hologram " + pack.getQuestPath() + "."
                            + key + ": " + e.getMessage());
                }
            }
        }
    ...
    private void updateHolograms() {
        ...
        for (final Map.Entry<Integer, List<NPCHologram>> entry : npcs.entrySet()) {
            for (final NPCHologram npcHologram : entry.getValue()) {
                final NPC npc = BetonQuest.simpleNPCs().getNPC(entry.getKey());
                if (npc == null) {
                    continue;
                }
        ...
        for (final Map.Entry<Integer, List<NPCHologram>> entry : npcs.entrySet()) {
            for (final NPCHologram npcHologram : entry.getValue()) {
                if (npcHologram.hologram != null) {
                    final NPC npc = BetonQuest.simpleNPCs().getNPC(entry.getKey());
                    if (npc != null) {
                        npcHologram.hologram.move(npc.getLocation().clone().add(npcHologram.vector));
                    }
                }
            }
        }
    ...
    private boolean updateHologramsForPlayers(final NPCHologram npcHologram, final NPC npc) {
        ...
        for (final OnlineProfile onlineProfile : PlayerConverter.getOnlineProfiles()) {
            if (BetonQuest.conditions(onlineProfile, npcHologram.conditions)) {
                ...
                if (npcHologram.hologram == null) {
                    final BetonHologram hologram = HologramProvider.getInstance().createHologram(
                            String.valueOf(BetonQuest.simpleNPCs().getID(npc)), npc.getLocation().add(npcHologram.vector));
