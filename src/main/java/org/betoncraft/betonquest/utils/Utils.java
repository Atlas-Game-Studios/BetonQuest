...
public final class Utils {
    ...
    public static boolean backupDatabase(final File databaseBackupFile) {
        ...
        try {
            ...
            final ConfigAccessor accessor = new ConfigAccessor(databaseBackupFile, databaseBackupFile.getName(), ConfigAccessor.AccessorType.OTHER);
            ...
            for (final String table : tables) {
                ...
                map.put(table, database.querySQL(Connector.QueryType.valueOf(enumName)));
            }
    ...
    public static void loadDatabaseFromBackup() {
        ...
        final ConfigAccessor accessor = new ConfigAccessor(file, "database-backup.yml", ConfigAccessor.AccessorType.OTHER);
        final FileConfiguration config = accessor.getConfig();
        ...
        con.updateSQL(Connector.UpdateType.DROP_OBJECTIVES);
        con.updateSQL(Connector.UpdateType.DROP_TAGS);
        con.updateSQL(Connector.UpdateType.DROP_POINTS);
        con.updateSQL(Connector.UpdateType.DROP_JOURNALS);
        con.updateSQL(Connector.UpdateType.DROP_PLAYER);
        // create new tables
        database.createTables(instance.isMySQLUsed());
        // load objectives
        ...
        if (objectives != null) {
            for (final String key : objectives.getKeys(false)) {
                con.updateSQL(Connector.UpdateType.INSERT_OBJECTIVE,
        ...
        if (tags != null) {
            for (final String key : tags.getKeys(false)) {
                con.updateSQL(Connector.UpdateType.INSERT_TAG, tags.getString(key + ".id"),
        ...
        if (points != null) {
            for (final String key : points.getKeys(false)) {
                con.updateSQL(Connector.UpdateType.INSERT_POINT,
        ...
        if (journals != null) {
            for (final String key : journals.getKeys(false)) {
                con.updateSQL(Connector.UpdateType.INSERT_JOURNAL,
        ...
        if (backpack != null) {
            for (final String key : backpack.getKeys(false)) {
                con.updateSQL(Connector.UpdateType.INSERT_BACKPACK,
        ...
        if (player != null) {
            for (final String key : player.getKeys(false)) {
                con.updateSQL(Connector.UpdateType.INSERT_PLAYER,
