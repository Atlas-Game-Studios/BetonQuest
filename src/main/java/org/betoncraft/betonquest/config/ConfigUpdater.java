package org.betoncraft.betonquest.config;

import edu.umd.cs.findbugs.annotations.SuppressFBWarnings;
import org.apache.commons.lang3.StringUtils;
import org.betoncraft.betonquest.BetonQuest;
import org.betoncraft.betonquest.config.ConfigAccessor.AccessorType;
import org.betoncraft.betonquest.database.Connector;
import org.betoncraft.betonquest.database.Connector.QueryType;
import org.betoncraft.betonquest.database.Connector.UpdateType;
import org.betoncraft.betonquest.database.Database;
import org.betoncraft.betonquest.database.Saver.Record;
...
public class ConfigUpdater {
            ...
            for (final ConfigPackage pack : Config.getPackages().values()) {
                ...
                for (final String locName : locList.split(",")) {
                    ...
                    instance.getSaver().add(new Record(UpdateType.RENAME_ALL_TAGS,
                            packName + ".global_" + locName, "global_" + locName));
                }
            }
        } catch (final Exception e) {
            LogUtils.getLogger().log(Level.WARNING, ERROR);
        ...
        try {
            ...
            for (final String packName : tags.keySet()) {
                for (final String tag : tags.get(packName)) {
                    instance.getSaver()
                            .add(new Record(UpdateType.RENAME_ALL_TAGS, packName + "." + tag, tag));
                }
            }
            for (final String packName : points.keySet()) {
                for (final String point : points.get(packName)) {
                    instance.getSaver().add(
                            new Record(UpdateType.RENAME_ALL_POINTS, packName + "." + point, point));
                }
            }
            for (final String packName : points.keySet()) {
                for (final String point : points.get(packName)) {
                    instance.getSaver().add(
                            new Record(UpdateType.RENAME_ALL_GLOBAL_POINTS, packName + "." + point, point));
                }
            }
        ...
        try {
            ...
            for (final String key : conditionsConfig.getKeys(false)) {
                ...
                if (instruction.startsWith("or ") || instruction.startsWith("and ")) {
                    ...
                    final String newInstruction = firstPart
                            + newConditionsList.substring(0, newConditionsList.length() - 1) + lastPart;
            ...
            for (final String conversationName : conversations.keySet()) {
                ...
                for (final String path : paths) {
                    ...
                    final String newList = newListBuilder.substring(0, newListBuilder.length() - 1);
    ...
    private void convertNamesToUUID() {
        ...
        for (final String table : tables) {
            final ResultSet res = con.querySQL(QueryType.valueOf("SELECT_PLAYERS_" + table));
            try {
                while (res.next()) {
