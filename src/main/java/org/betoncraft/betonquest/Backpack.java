...
import org.betoncraft.betonquest.api.QuestCompassTargetChangeEvent;
import org.betoncraft.betonquest.config.Config;
import org.betoncraft.betonquest.config.ConfigPackage;
import org.betoncraft.betonquest.config.QuestCanceler;
    ...
    private class Page extends Display {
        ...
        public Page(final int page) {
            ...
            if (page > 0) {
                ItemStack previous;
                try {
                    previous = new QuestItem(new org.betoncraft.betonquest.id.ItemID(Config.getDefaultPackage(), "previous_button")).generate(1);
            ...
            if (backpackItems.size() > (page + 1) * 45 - 1) {
                ItemStack next;
                try {
                    next = new QuestItem(new org.betoncraft.betonquest.id.ItemID(Config.getDefaultPackage(), "next_button")).generate(1);
                } catch (ObjectNotFoundException e) {
                    LogUtils.getLogger().log(Level.WARNING, "Could not find item: " + e.getMessage());
                    LogUtils.logThrowable(e);
            ...
            ItemStack cancel;
            try {
                cancel = new QuestItem(new org.betoncraft.betonquest.id.ItemID(Config.getDefaultPackage(), "cancel_button")).generate(1);
            } catch (ObjectNotFoundException e) {
            ...
            try {
                compassItem = new QuestItem(new org.betoncraft.betonquest.id.ItemID(Config.getDefaultPackage(), "compass_button")).generate(1);
            } catch (ObjectNotFoundException e) {
                LogUtils.getLogger().log(Level.WARNING, "Could not find item: " + e.getMessage());
                LogUtils.logThrowable(e);
